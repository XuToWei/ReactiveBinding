...
3. **ReactiveDataModels** (`ReactiveDataModels.cs`) - Data structures: `ReactiveClassData`, `ReactiveSourceData`, `ReactiveBindData`
4. **DiagnosticDescriptors** (`DiagnosticDescriptors.cs`) - All diagnostic codes (RB0xxx warnings, RB1xxx class errors, RB2xxx source errors, RB3xxx binding errors)

### Code Generation Flow

1. `ReactiveSyntaxReceiver` collects members with `[ReactiveSource]` and methods with `[ReactiveBind]`
2. Generator validates: partial class, implements `IReactiveObserver`, source/binding constraints
3. Generates partial class with:

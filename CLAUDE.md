...
- **VersionProtocolAccessAnalyzer** - Prevents user code from accessing reserved `IVersion`/`IVersionSync` `__*` protocol members and `__*` members emitted by `ReactiveBindGenerator` for an `IReactiveObserver` (VF10012), while allowing generated code and exact runtime interfaces/kernel/container types; handwritten `__*` members on a plain `IReactiveObserver` are not claimed
